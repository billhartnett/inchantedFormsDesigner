...
export interface AcordLabel {
  ...
  name: string;
  description: string;
  category: AcordCategory;
  fieldType: AcordFieldType;
  examples: string[];
  pattern?: RegExp;
  priority: number; // Higher = more likely to be suggested
}

export enum AcordCategory {
  INSURED = "Insured",
  PRODUCER = "Producer",
  POLICY = "Policy",
  COVERAGE = "Coverage",
...
export interface LabelSuggestion {
  label: AcordLabel;
  score: number; // 0-1, combination of vector and rule-based scores
...
export interface AcordSuggestionRequest {
  fields: FieldContent[];
  documentType?: string; // e.g., "homeowners", "commercial", "auto"
  topK?: number; // Number of suggestions per field (default: 3)
}

export interface AcordSuggestionResponse {
  success: boolean;
  documentType?: string;
...
export const ACORD_LABELS: AcordLabel[] = [
  // Insured Information
  {
