...
import {
  AcordLabel,
  FieldContent,
  AcordSuggestionRequest,
  AcordSuggestionResponse,
  SuggestionResult,
  LabelSuggestion,
  ACORD_LABELS,
} from "./acordModels";
...
export class LabelSuggestionService {
   ...
   */
  async suggestLabels(request: AcordSuggestionRequest): Promise<AcordSuggestionResponse> {
    const startTime = Date.now();
    const modelsUsed = new Set<string>(["embeddings", "vector-search", "rules"]);
    ...
    try {
      ...
      for (let idx = 0; idx < request.fields.length; idx++) {
        ...
        const result = await this.suggestLabelsForField(
          field,
          request.topK || 3,
          idx,
   ...
   */
  private async suggestLabelsForField(
    field: FieldContent,
    topK: number,
    fieldIndex: number,
    totalFields: number
    ...
    try {
      ...
      const splitField = MultiFieldSplitter.splitField(field.content, field.fieldType);
      reasoning.push(
        `Detected patterns: ${splitField.detectedPatterns.join(", ") || "text"}`
      );

      // Step 2: Get vector search scores using embeddings
      reasoning.push("Performing semantic search...");
      const vectorScores = await this.embeddingsService.rankLabels(
        field.fieldName,
        field.content,
        topK * 3 // Get more candidates for filtering
      );
      ...
      const ruleResult = RuleBasedProcessor.processRules(
        field.fieldName,
        field.content,
        vectorScores,
        fieldIndex,
      ...
      if (field.fieldType) {
        ...
        finalScores = RuleBasedProcessor.filterByFieldType(
          finalScores,
          field.fieldType
        );
      }
  ...
  private createSuggestions(
    ...
    finalScores.forEach((ruleScore, labelId) => {
      ...
      // Determine method
      let method: "vector" | "rule-based" | "pattern" | "hybrid" = "vector";
   ...
   */
  async getSuggestionsWithAnalysis(
    request: AcordSuggestionRequest
  ): Promise<AcordSuggestionResponse & { analysis: string }> {
   ...
   */
  private generateAnalysis(suggestions: SuggestionResult[], documentType?: string): string {
    const lines: string[] = [];
    ...
    suggestions.forEach((result, idx) => {
      ...
      if (result.suggestedLabels.length > 0) {
        ...
        result.suggestedLabels.forEach((suggestion, idx) => {
          lines.push(
            `  ${idx + 1}. ${suggestion.label.name} (${suggestion.method}) - ${(suggestion.score * 100).toFixed(1)}%`
          );
