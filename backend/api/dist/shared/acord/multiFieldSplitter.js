...
exports.MultiFieldSplitter = exports.ComponentType = void 0;
var ComponentType;
...
class MultiFieldSplitter {
     ...
     */
    static parseAddress(content) {
        // Simple address parsing for: "123 Main St, City, ST 12345"
        const addressPattern = /^(\d+\s+[^,]+),?\s*([^,]+),?\s*([A-Z]{2})\s+(\d{5}(?:-\d{4})?)?/;
    ...
    static parseAmount(content) {
        ...
        const amountPattern = /(\$?\s*[\d,]+\.?\d{0,2}(?:\s*(?:USD|dollars|cents))?)/i;
     ...
     */
    static getContext(content) {
        const split = this.splitField(content);
        return split.detectedPatterns;
    }
}
exports.MultiFieldSplitter = MultiFieldSplitter;
